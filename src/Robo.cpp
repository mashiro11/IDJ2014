...
Robo::Robo(float x, float y, string file, GameObject& associated):
    associated(associated),
    sp(*(new Sprite(associated, file, x, y, ROBO_SHEET_FRAMES, ROBO_SHEET_LINES, ROBO_SHEET_FRAME_TIME))),
    selected(false)
{
    DEBUG_PRINT("Robo::Robo()-inicio");

    associated.AddComponent((Component&)sp);
    ...
    associated.AddComponent((Component&)barraVida);
    ...
    associated.AddComponent((Component&)coolDownBar);

    Walkable& walkable = *(new Walkable(associated));
    walkable.SetControlable(true);
    associated.AddComponent((Component&) walkable);
